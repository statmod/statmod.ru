...
<xsl:variable name="root_path" select="'http://statmod.ru'"/>
<xsl:variable name="root_path_xml" select="concat($root_path,$path_xml)"/>
...
<xsl:if test="$need_resolve_links and not (starts-with(@src, '/') or 
                                           ...
                                           starts-with(@src, '#'))">
  <xsl:attribute name="src"><xsl:value-of select="concat($root_path, $path_xml,@src)"/></xsl:attribute>
</xsl:if>
...
<xsl:if test="$need_resolve_links and boolean(@href) and not (starts-with(@href, '/') or 
                                           ...
                                           starts-with(@href, '#') or 
                                           starts-with(@href, '?'))">
  <xsl:attribute name="href"><xsl:value-of select="concat($root_path, $path_xml,@href)"/></xsl:attribute>
</xsl:if>
